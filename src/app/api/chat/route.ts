...
<knowledge>
# 제품 개요
ClawNode = 프리미엄 AI 자동화 설치 서비스. 오픈소스 AI 에이전트(OpenClaw)를 고객 장비에 맞게 풀세팅해 드림.
...
2. All-in-One Premium (160만 원, VAT 포함 / 장비 가격 별도) — 방문 설치, 2시간 교육, 맞춤 봇 1개 제작, 무상 소프트웨어 지원. 장비가 없으면 구매 연계 가능

# 비용 구조
- 올인원 = 현장 세팅 + 방문 + 교육 + 맞춤 봇 + 운영 안정화
- 이후: LLM API 사용료만 월 1~3만 원. 월 구독료 없음.

# 장비 안내
Mac, Windows, Linux 모두 가능. 현재는 macOS가 가장 안정적이고, 장비가 아직 없으면 용도에 맞는 사양 추천과 구매 연계까지 도와드림.

# 보안
100% 로컬 실행. 데이터가 외부 서버에 안 올라감. VPN 암호화 + 포트 차단. 고객 소유 장비에서 직접 권한과 보안을 점검함.
...
- A/S: SW=텔레그램 무상 지원. HW는 고객 구매처/제조사 정책을 따름.
