...
# ============================================================================
# OpenClaw Quick Setup v6 — Core Memory Stack + Z.AI Sanitizer
#
# v5 -> v6 migration:
#   Added Z.AI provider sanitizer preload support.
#   When ZAI_SELECTED=1, installs a Node.js --require preload script into the
#   gateway LaunchAgent to sanitize outbound requests for Z.AI compatibility.
#
# Scope:
...
#   7. Gateway restart + Z.AI sanitizer preload + health check
...
LOG_FILE="${CONFIG_DIR}/setup-v6-$(date +%Y%m%d-%H%M%S).log"
CLAWNODE_VERSION_FILE="${CONFIG_DIR}/.clawnode-version"
WIKI_VAULT_PATH="${CONFIG_DIR}/wiki/main"
ZAI_SANITIZER_GIST_URL="https://gist.githubusercontent.com/VictorJeon/e3207ff026158cb9e0c4a4e9d1c8cd7f/raw/zai_openclaw_sanitize.js"
ZAI_SANITIZER_SOURCE="${REPO_ROOT}/installer/assets/zai_openclaw_sanitize.js"
ZAI_SANITIZER_DIR="${CONFIG_DIR}/scripts"
ZAI_SANITIZER_DEST="${ZAI_SANITIZER_DIR}/zai_openclaw_sanitize.js"
ZAI_API_KEY=""
ZAI_PROVIDER_SET=0
...
print_hero() {
  echo ""
  echo "============================================================"
  printf '  %b\n' "${BOLD}OpenClaw V6 — Core Memory Stack${NC}"
...
write_log_header() {
  echo "# OpenClaw Setup V6 Log — $(date)"
...
render_final_summary() {
  ...
  report="OpenClaw V6 Setup Result
...
Workspace: ${WORKSPACE}
Report: ${CONFIG_DIR}/install-report-v6.txt
Log: ${LOG_FILE}"
...
Tailscale IP: ${ts_ip}"
  ...
  report_file="${CONFIG_DIR}/install-report-v6.txt"
  printf '%s\n' "${report}" > "${report_file}"

  echo ""
  echo "============================================================"
  printf '  %b\n' "${GREEN}${BOLD}OpenClaw V6 — Core Memory Stack Ready${NC}"
  echo "============================================================"
  echo ""
...
PYEOF
  ...
  case "${provider}" in
    ...
    google|gemini)
      ...
      ;;
    zai)
      curl -fsS --max-time 15 -H "Authorization: Bearer ${secret}" "https://api.z.ai/api/coding/paas/v4/models" -o /dev/null 2>/dev/null && return 0
      ;;
    *) return 1 ;;
  esac
...
run_core_setup() {
  ...
  if [[ "${FORCE_CORE_SETUP}" != "1" ]] && core_install_present; then
    ...
    warn "existing core found but auth invalid — re-authenticate."
    UPDATE_MODE=1
...
# ============================================================================
# Z.AI Provider Setup (Feature 1)
# ============================================================================

prompt_zai_provider() {
  echo ""
  echo "  ${BOLD}Optional: Z.AI (GLM-5.1) Provider${NC}"
  echo "  Add Z.AI Coding Plan Global as your primary model?"
  echo ""
  echo "  [y] Yes — enter Z.AI API key"
  echo "  [N] No — skip"
  echo ""
  read -rp "  Install Z.AI provider? [y/N]: " ZAI_CHOICE
  case "${ZAI_CHOICE,,}" in
    y|yes)
      read -rsp "  Z.AI API Key: " ZAI_API_KEY
      echo ""
      if [[ -z "${ZAI_API_KEY}" || ${#ZAI_API_KEY} -lt 20 ]]; then
        warn "Z.AI API key too short — skipping provider setup."
        ZAI_API_KEY=""
        return 1
      fi
      ZAI_SELECTED=1
      ZAI_PROVIDER_SET=1
      ok "Z.AI API key received."
      ;;
    *)
      ZAI_API_KEY=""
      ;;
...
apply_zai_config() {
  if [[ "${ZAI_PROVIDER_SET}" != "1" || -z "${ZAI_API_KEY}" ]]; then return 0; fi
  info "applying Z.AI provider config"

  if [[ "${DRY_RUN}" == "1" ]]; then
    ok "[DRY] Z.AI: models.providers.zai + auth-profiles + auth.order"
    return 0
  fi

  # 1. Patch openclaw.json: models.providers.zai + agents.defaults.model
  python3 - <<'PYEOF'
import json, os
...
c["models"]["providers"]["zai"] = {
    "baseUrl": "https://api.z.ai/api/coding/paas/v4"
}
c.setdefault("agents", {}).setdefault("defaults", {}).setdefault("model", {})
c["agents"]["defaults"]["model"]["primary"] = "zai/glm-5.1"
with open(config_path, "w") as f:
    json.dump(c, f, indent=2)
print("OK: models.providers.zai + agents.defaults.model.primary")
PYEOF

  # 2. Write auth-profiles.json
  local auth_dir="${CONFIG_DIR}/agents/main/agent"
  mkdir -p "${auth_dir}"
  AUTH_FILE="${auth_dir}/auth-profiles.json" ZAI_API_KEY="${ZAI_API_KEY}" python3 - <<'PYEOF'
import json, os
...
print("OK: auth-profiles zai:default")
PYEOF

  # 3. Patch auth.order in openclaw.json (provider-keyed)
  python3 - <<'PYEOF'
import json, os
config_path = os.path.expanduser("~/.openclaw/openclaw.json")
with open(config_path) as f:
    c = json.load(f)
c.setdefault("auth", {}).setdefault("order", {})
c["auth"]["order"]["zai"] = ["zai:default"]
with open(config_path, "w") as f:
    json.dump(c, f, indent=2)
print("OK: auth.order.zai")
PYEOF

  # 4. Save to Keychain
  security add-generic-password -U -s "openclaw-zai-api" -a "zai" -w "${ZAI_API_KEY}" 2>/dev/null
  ok "Z.AI provider config applied."
}

# Feature 3: Auth re-auth menu (instead of forcing core re-run)
prompt_reauth_provider() {
  echo ""
  echo "  ${BOLD}Auth Validation Failed — Re-authenticate${NC}"
  echo ""
  ...
  echo ""
  read -rp "  Choice [1-4]: " REAUTH_CHOICE
  case "${REAUTH_CHOICE}" in
    1)
      # Delegate to core script's Anthropic path
      SUPPRESS_FINAL_REPORT=1 OPENCLAW_PARENT_LOG=1 bash "${CORE_SCRIPT}"
      ;;
    2)
...
PYEOF
      ;;
    3)
      read -rsp "  Z.AI API Key: " REAUTH_KEY
      echo ""
      ZAI_API_KEY="${REAUTH_KEY}"
      ZAI_PROVIDER_SET=1
      apply_zai_config
      ;;
    *)
...
# ============================================================================
# Z.AI Sanitizer Preload
# ============================================================================

install_zai_sanitizer() {
  if [[ "${ZAI_SELECTED}" != "1" ]]; then
    return 0
  fi
  info "Z.AI sanitizer: installing preload script"

  # Try local repo first, fall back to Gist download
  local sanitizer_src="${ZAI_SANITIZER_SOURCE}"
  if [[ ! -f "${sanitizer_src}" ]]; then
    info "local sanitizer not found — downloading from Gist"
    sanitizer_src="${CONFIG_DIR}/scripts/.zai_sanitize_download.js"
    if ! curl -fsSL --max-time 15 "${ZAI_SANITIZER_GIST_URL}" -o "${sanitizer_src}" 2>/dev/null; then
      err "failed to download sanitizer from Gist"
      return 1
    fi
  fi

  if [[ "${DRY_RUN}" == "1" ]]; then
    ok "[DRY] cp ${sanitizer_src} -> ${ZAI_SANITIZER_DEST}"
    return 0
  fi

  mkdir -p "${ZAI_SANITIZER_DIR}"
  cp "${sanitizer_src}" "${ZAI_SANITIZER_DEST}"
  chmod 644 "${ZAI_SANITIZER_DEST}"
  # Clean up temp download
  if [[ "${sanitizer_src}" != "${ZAI_SANITIZER_SOURCE}" ]]; then
    rm -f "${sanitizer_src}"
  fi
  ok "sanitizer installed: ${ZAI_SANITIZER_DEST}"
}

patch_gateway_zai_node_options() {
  if [[ "${ZAI_SELECTED}" != "1" ]]; then
    return 0
  fi
  ...
  if [[ ! -f "${gateway_plist}" ]]; then
    warn "gateway plist not found — skipping Z.AI NODE_OPTIONS patch"
    return 0
  fi

  info "Z.AI sanitizer: patching gateway NODE_OPTIONS"

  if [[ "${DRY_RUN}" == "1" ]]; then
    ok "[DRY] PlistBuddy NODE_OPTIONS --require ${ZAI_SANITIZER_DEST}"
    return 0
  fi

  local require_fragment="--require ${ZAI_SANITIZER_DEST}"

  # Ensure EnvironmentVariables dict exists
  if ! /usr/libexec/PlistBuddy -c 'Print :EnvironmentVariables' "${gateway_plist}" >/dev/null 2>&1; then
    /usr/libexec/PlistBuddy -c 'Add :EnvironmentVariables dict' "${gateway_plist}" >/dev/null 2>&1
  fi

  local existing_node_opts
  existing_node_opts="$(/usr/libexec/PlistBuddy -c 'Print :EnvironmentVariables:NODE_OPTIONS' "${gateway_plist}" 2>/dev/null || true)"

  local new_value
  if [[ -z "${existing_node_opts}" ]]; then
    # NODE_OPTIONS not present — set it to just the require fragment
    new_value="${require_fragment}"
    /usr/libexec/PlistBuddy -c "Add :EnvironmentVariables:NODE_OPTIONS string \"${new_value}\"" "${gateway_plist}" >/dev/null 2>&1
  elif [[ "${existing_node_opts}" == *"${require_fragment}"* ]]; then
    # Already contains the exact --require <path> fragment — idempotent skip
    ok "NODE_OPTIONS already contains sanitizer preload"
    return 0
  else
    # Append to existing value, preserving all prior flags
    new_value="${existing_node_opts} ${require_fragment}"
    /usr/libexec/PlistBuddy -c "Set :EnvironmentVariables:NODE_OPTIONS \"${new_value}\"" "${gateway_plist}" >/dev/null 2>&1
  fi

  ok "NODE_OPTIONS patched: ${new_value}"

  # Re-bootstrap gateway so it picks up the new environment
  local uid
  uid="$(id -u)"
  local svc_label="ai.openclaw.gateway"

  launchctl bootout "gui/${uid}/${svc_label}" 2>/dev/null || true
  sleep 1
  launchctl bootstrap "gui/${uid}" "${gateway_plist}" 2>/dev/null || true
  launchctl kickstart -kp "gui/${uid}/${svc_label}" 2>/dev/null || true

  if wait_for_gateway_health 15; then
    ok "gateway restarted with Z.AI sanitizer preload"
  else
    warn "gateway health check failed after Z.AI patch — verify manually"
  fi
}
...
INSTALLER_URL=${INSTALLER_V6_URL}
STAMPEOF
...
main() {
  ...
  # Apply Z.AI config after core setup (gateway + config exist now)
  stage "Z.AI Provider"
  apply_zai_config
  ...
  ok "setup v6 core memory stack complete"
}
